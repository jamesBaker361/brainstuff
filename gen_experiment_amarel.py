    ...
    for translate in ["trans","untrans"]:
        for recons in ["recons","unrecons"]:
            for disc in ["disc","no_disc"]:
                ...
                command=f" sbatch -J cycle --err=slurm/cycle/{name}.err --out=slurm/cycle/{name}.out --exclude=gpu[005,006,008,010-014,017,018],cuda[001-008],pascal[001-010],gpuk[001-012] runpygpu.sh "
                command+=" cycle_training.py --epochs 1000 --validation_interval 50 --project_name cycle_sub1 --sublist 1 --fmri_type array "
                command+=" --gradient_accumulation_steps 16 --mixed_precision fp16 "
                if pairing=="paired":
