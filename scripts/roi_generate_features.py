...
with open(os.environ["BRAIN_DATA_DIR"]+'/regression_weights/subj{:02d}/vdvae_regression_weights.pkl'.format(sub),"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight']
    reg_b = datadict['bias']

roi_dir = os.environ["BRAIN_DATA_DIR"]+'/processed_data/subj{:02d}/roi'.format(sub)
num_rois = 13
...
np.save(os.environ["BRAIN_DATA_DIR"]+'/predicted_features/subj{:02d}/nsd_vdvae_nsdgeneral_roi_sub{}_31l_alpha50k.npy'.format(sub,sub),pred_vae)

# Generate CLIP-Text Features

with open(os.environ["BRAIN_DATA_DIR"]+'/regression_weights/subj{:02d}/cliptext_regression_weights.pkl'.format(sub),"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight']
    reg_b = datadict['bias']
...
np.save(os.environ["BRAIN_DATA_DIR"]+'/predicted_features/subj{:02d}/nsd_cliptext_roi_nsdgeneral.npy'.format(sub),pred_clipt)

# Generate CLIP-Vision Features

with open(os.environ["BRAIN_DATA_DIR"]+'/regression_weights/subj{:02d}/clipvision_regression_weights.pkl'.format(sub),"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight']
    reg_b = datadict['bias']
...
np.save(os.environ["BRAIN_DATA_DIR"]+'/predicted_features/subj{:02d}/nsd_clipvision_roi_nsdgeneral.npy'.format(sub),pred_clipv)
